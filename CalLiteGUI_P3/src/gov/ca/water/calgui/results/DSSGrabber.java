...
public class DSSGrabber {
	...
	private Project project = ResultUtils.getXMLParsingSvcImplInstance(null).getProject();
	...
	public void setLocation(String locationName) {
		...
		if (locationName.startsWith("/")) {
			...
			for (int i = 0; i < ResultUtils.getXMLParsingSvcImplInstance(null).getLookups5Length(); i++) {
				if (locationName.toUpperCase().endsWith(ResultUtils.getXMLParsingSvcImplInstance(null).getLookups5(i, 0))) {
		...
		try {
			...
			if ((result == null) || (result.numberValues < 1)) {
				...
				if (!clsIsDynamicSJR(dssFilename) && ((dssNames[0].equals("S_MELON/STORAGE"))
				        ...
				        || (dssNames[0].equals("C_SJRMS/FLOW-CHANNEL")) || (dssNames[0].equals("D_STANRIPN/FLOW-DELIVERY"))
				        ...
				        || (dssNames[0].equals("D_MERCED1/FLOW-DELIVERY")) || (dssNames[0].equals("D_MERCED2/FLOW-DELIVERY"))
				...
				else if (!clsLVE(dssFilename) && ((dssNames[0].equals("S422/STORAGE")) || (dssNames[0].equals("WQ408_OR_/SALINITY"))
				...
				for (int i = 1; i < dssNames.length; i++) {
					...
					TimeSeriesContainer result2 = (TimeSeriesContainer) hD
					        .get("/" + hecAPart + "/" + dssNames[i] + "/01JAN2020/1MON/" + hecFPart, true);
					if (result2 == null) {
						JOptionPane.showMessageDialog(null, "Could not find " + dssNames[0] + " in " + dssFilename, "Error",
						        JOptionPane.ERROR_MESSAGE);
					} else {
